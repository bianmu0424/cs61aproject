        ...
        while current_place:
            ...
            if ant:
                if not hasattr(ant, 'damage_doubled'):
                    ant.damage *= 2
                    setattr(ant, 'damage_doubled', True)
                if hasattr(ant, 'is_container') and ant.is_container:
                    contained_ant = ant.ant_contained
                    if contained_ant and not hasattr(contained_ant, 'damage_doubled'):
                        contained_ant.damage *= 2
                        setattr(contained_ant, 'damage_doubled', True)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        flag = self.place
        while not flag.is_hive:
            if distance <= self.upper_bound and distance >= self.lower_bound and flag.bees:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        insects = {}
        place = self.place  # 开始的位置
        distance = 0  # 初始距离为0
        while place is not None and not place.is_hive: # 沿着前方路径遍历，直到达到Hive
            for bee in place.bees:
                insects[bee] = distance
            if place.ant is self:  # 跳过LaserAnt本身
                distance += 1
                place = place.entrance
                continue
            # 如果这个位置上有蚂蚁，并且它不是激光蚂蚁自己
            if place.ant and place.ant is not self:
                insects[place.ant] = distance
            place = place.entrance  # 移动到下一个位置
            distance += 1  # 增加距离
    ...
    def action(self, gamestate):
        ...
        for insect, distance in insects_and_distances.items():
            hurt = self.calculate_damage(distance)
            insect.reduce_health(hurt)
            if hurt:
                self.insects_shot += 1
