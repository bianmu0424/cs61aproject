...
def about(subject):
    ...
    def test(paragraph):
        ...
        for word in words:
            if remove_punctuation(word) in subject:
                return True
        return False
    return test
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    lens=len(source_words)
    if (lent==0 and lens!=0) or (lent!=0 and lens==0):
        return 0.0
        ...
        if lent<=lens:
            k=0
            i=0
            for elem in typed_words:
                if elem==source_words[i]:
                    k+=1
                    i+=1
                else:
                    i+=1
            return round(k/lent*100,2)
        else:
            k = 0
            i = 0
            for elem in typed_words:
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        happen=False
        i,flag,min=0,0,limit+1
        for word in word_list:
            if diff_function(typed_word,word,limit)<=limit:
                if min>diff_function(typed_word,word,limit):
...
def minimum_mewtations(typed, source, limit):
    ...
    '''if len(typed) >= 2 and len(source) >= 2:
        if typed[-1] == source[-2] and typed[-2] == source[-1]:
            return minimum_mewtations(typed[:-2], source[:-2], limit - 1) final_diff专供'''
...
def fastest_words(match):
    ...
    for word_index in word_indices:
        ...
        fastest_words_per_player[fastest_player].append(word)
