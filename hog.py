...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    # BEGIN PROBLEM 9
    k=1
    maix= make_averaged(roll_dice,samples_count)
    flag=maix(1,dice)
    for i in range(2, 11):
        new = maix(i,dice)
        if flag < new:
...
def final_strategy(score, opponent_score):
    ...
    x=0
    for i in range(0,3):
        if k<=sus_update(i, score, opponent_score):
            k=sus_update(i, score, opponent_score)
            x=i
    if score+k>=100:
        return x
    trust=make_averaged(sus_update,samples_count=1)
    tr=trust(0,score,opponent_score)
    ordinary=make_averaged(roll_dice,samples_count=6)
    oy=ordinary(6)
    if tr>=oy:
        return 0
